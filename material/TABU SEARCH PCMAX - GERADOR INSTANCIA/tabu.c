...
#define MAXN 400 /* Maximum tasks' number plus processors */ // original 286
...
double wtime()
{
  struct timeval t;
  gettimeofday(&t, NULL);
  return t.tv_sec * 1000000.0 + t.tv_usec * 1.0;
...
{
  //----inserido por Fernando------
  double START, END;
  int outTime = 0;
  START = wtime();
  double timeViz = -100000;
  ...
  for (alfa = 0.5; alfa <= .8; alfa += .5)
  {
    ...
    for (iteration = 1; imptrue; iteration++)
    {
          ...
          if (j != procplus)
          {
                ...
                (depot.tabt[j].nbredenoeuds <= 1))
            {
              ...
              if ((depot.load[j] + flag.mdelta) <= minpm)
              {
                ...
                {
                  if (((depot.load[j] + flag.mdelta) < makespan.load) && (minpl < makespan.load))
                  {
                    aspiration = 1;
                    for (k = 1; k <= np; k++)
                    {
      ...
      {
        printf("\nmakespan = %d - %8.2f [%d]", maxload, timetab, iteration);
      ...
      { //{xxxxxx
        if ((iteration - lastimp) > (n * 10) && outTime == 0)
        {
          timeViz += timetab;
          outTime = 1;
          printf(" ## in %4.2f\n", timetab);
          if (timeViz > 0)
            printf("---- Usando tempo extra predeterminado ... ----\n");
        }
    ...
    fprintf(outf, "%5d%5d%5d%5d%5d%5d\n", (int)nprob, (int)ns, (int)np, (int)eucl, (int)choix, (int)maxdist);
    for (i = 1; i <= n; i++)
    {
      for (j = (i + 1); j <= (n + 1); j++)
      {
  ...
  /*** CREATION OF THE OUTPUT FILE TO VIZINHANÇA DE LARGA ESCALA ***/
  ...
  fprintf(outf, "%s %d %d e\n", fname, np, n); // primeira linha
  for (j = 1; j <= np; j++)
  {
    imppb = bsolution.tabt[j].ptr;
    for (i = 1; i <= bsolution.tabt[j].nbredenoeuds; i++)
    {
      if (imppb->noeud <= n)
      {
        fprintf(outf, "%5d %5d %5d\n", i - 2, imppb->noeud - 1, j - 1);
      }
      imppb = imppb->prochain;
    }
  }
  fprintf(outf, "-1 -1 -1\n%30s   time: %10.3f  fo: %10d\n", fname, timetab, makespan.load);
  fclose(outf);
  printf("\n");
} /*  FIM MAIN()  */
      ...
      if (neigh->p1[var].nn[j] == x)
      {
        ...
        else
        {
          ...
          if (neigh->p1[var].leplusloin == j)
            ...
            sofar = neigh->p1[var].nn[neigh->p1[var].leplusloin];
      ...
      if (neigh->p2[var].nn[j] == x)
      {
        ...
        else
        {
          ...
          if (neigh->p2[var].leplusloin == j)
            ...
            sofar = neigh->p2[var].nn[neigh->p2[var].leplusloin];
...

{
  ...
  if (xk1 <= 2)
  {
    ...
    if (xk1 == 1)
      ...
      nouvdelta = dtp[ni][nj] - dtp[ni][x] - dtp[x][nj];
    if (nouvdelta <= delta)
    {
      delta = nouvdelta;
